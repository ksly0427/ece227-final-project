...
def main(args):
    ...
    # Add new parameters for mask mandate
    mask_perc = args.mask_perc
    mask_effectiveness = args.mask_effectiveness
    mask_start_date = datetime.date(2021,1,20)
    mask_end_date = datetime.date(2021,4,27)
    mask_fall_off = args.mask_fall_off

    if args.mask_start_date:
        mask_start_date = str_to_date(args.mask_start_date)

    if args.mask_end_date:
        mask_end_date = str_to_date(args.mask_end_date)
    ...
    # Handle new mask mandate parameters
    if mask_perc > 0:
        assert mask_effectiveness in [0.1, 0.25, 0.5, 0.75], \
            ('must specify --quarantine_effectiveness percentage.'
                ' Possible values: [0.025, 0.1, 0.25, 0.5]')
        mask_effectiveness_to_reduction_idx = {0.1: 0, 0.25: 1, 0.5: 2, 0.75: 3}
        region_model.mask_fraction = mask_perc
        region_model.mask_reduction_idx = mask_effectiveness_to_reduction_idx[mask_effectiveness]
        
        region_model.mask_start_date_ind = (mask_start_date - region_model.first_date).days
        region_model.mask_end_date_ind = (mask_end_date - region_model.first_date).days
        region_model.mask_fall_off = mask_fall_off
...
if __name__ == '__main__':
    ...
    # Add new parameters for mask mandate
    parser.add_argument('--mask_perc', type=float, default=0,
        help=(''))
    parser.add_argument('--mask_effectiveness', type=float, default=-1,
        help=(''))
    parser.add_argument('--mask_start_date',
        help=(''))
    parser.add_argument('--mask_end_date',
        help=(''))
    parser.add_argument('--mask_fall_off', type=float, default=0.5,
        help=(''))
